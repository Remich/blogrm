$(document).ready(function() {
	...
	/**
	 * finds and returns the all types of editable elements and its values of a particular entry (e.g. blog post #12)
	 *
	 * TODO
	...
	var editable_on_blur = function(e) {
       if(editing != null && editing != 23) 
               editing.removeClass('editing').addClass('editable').removeAttr("contenteditable");
   }

			

	// $(document).on("click", ".editable", function(e) {	
	var editable_on_click = function(e) {   
...
$(document).on("click", ".editable", editable_on_click );
$(document).on("blur", ".editing", editable_on_blur );
...
String.prototype.replaceAll = function(search, replacement) {
	...
	$(document).on('click', '#pp_editor #edithtml', function(e) { 
		...
		if(state) { // apply normal displaying
			...
			// $(editing)..focus();
			$(editing).css('white-space', 'pre-wrap').text(value).focus();
		}
